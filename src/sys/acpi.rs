...
pub fn shutdown() {
    ...
    log!("ACPI Shutdown\n");
    let mut aml = AmlContext::new(Box::new(SsposAmlHandler), DebugVerbosity::None);
    let res = unsafe { AcpiTables::search_for_rsdp_bios(SsposAcpiHandler) };
    match res {
        Ok(acpi) => {
...
#[derive(Clone)]
pub struct SsposAcpiHandler;

impl AcpiHandler for SsposAcpiHandler {
...
struct SsposAmlHandler;

impl Handler for SsposAmlHandler {
